...
<div class="disclaimer d-flex justify-content-center toggle-quit">
    ...
    <div class="text">
        ...
        <h4 class="">
            <center><?=$encuesta['descripcion']?></center>
        </h4>
        <?php } ?>
    </div>
</div>
...
<main class="px-3 pb-5 mb-5 container toggle-quit">
    ...
    <form action="#" autocomplete="off">
       ...
       ?>
        ...
        <div class="card mt-2 pregunta" name="pregunta_<?=$regpre['id']?>" id="<?=$regpre['id']?>"
            type="<?=$regpre['tipo']?>" reque="<?=$regpre['requerido']?>">
            <!-- Pregunta -->
            ...
            <div class="card-body pt-0">
                <div class="row <?=$cssrow?>">
                    ...
                    <div class="col-4"></div>
                    <div class="col-4">
                        <input id="inputpreg_<?=$regpre['id']?>" name="inputpreg_<?=$regpre['id']?>"
                            class="form-control" placeholder="C.P." autocomplete="off"></input>
                    </div>
                    <div class="col-4"></div>
                    <?php } else { ?>
                    <textarea id="inputpreg_<?=$regpre['id']?>" name="inputpreg_<?=$regpre['id']?>" rows="2"
                        class="form-control" style="width: 90%;margin-left: 5%;" placeholder="Coloque su respuesta..."
                    ...
                    <div class="<?=$csscol?>">
                        <select class="form-select" id="select_<?=$regpre['id']?>">
                            ...
                            <!-- Opciones Select -->
                            <?php if ($regpre['tipo']=='SELECT') { ?>
                            <option value="<?=$regopc['id']?>" input="<?=$regopc['input']?>"
                                reque="<?=$regopc['requerido']?>"><?=$regopc['opcion']?></option>
                            <?php } else { ?>
                    ...
                    ?>
                            <div class="<?=$col?> d-flex justify-content-center">
                                <label class="mouse_select">
                                    ...
                                    <input type="<?=$tipo?>" name="pregunta_<?=$regpre['id']?>"
                                        input="<?=$regopc['input']?>" reque="<?=$regopc['requerido']?>"
                                        value="<?=$regopc['id']?>" id="<?=$regopc['id']?>"
                                        onclick="opcOtroCheckClick(<?php echo $regopc['id']; ?>);" autocomplete="off" />
                                    <?php } else { ?>
                                    <input type="<?=$tipo?>" name="pregunta_<?=$regpre['id']?>"
                                        input="<?=$regopc['input']?>" reque="<?=$regopc['requerido']?>"
                                        value="<?=$regopc['id']?>" id="<?=$regopc['id']?>"
                                        onclick="opcOtroRadioClick(<?php echo $regpre['id'].','.$regopc['id']; ?>);"
                                        autocomplete="off" />
                                    <?php } ?>
                                    <div class="option">
                                        <img class="img-fluid <?=$css?>"
                                            src="<?=$url_base;?><?=$upload_dir;?>/<?=$imagen;?>"
                                            alt="<?=$regopc['img_opcion'];?>">
                                        <div><?=$regopc['opcion']?></div>
                                        <!-- Opciones tiene input -->
                                        <?php if ($regopc['input']=='S') { ?>
                                        <input type="text"
                                            class="form-control  form-control-sm d-inline hide inputs<?=$regpre['id']?>"
                                            style="width: auto" placeholder="Coloque su respuesta..."
                                            id="input_<?=$regopc['id']?>" autocomplete="off" />
                                        <div class="invalid-feedback" id="msg_<?=$regpre['id']?>_<?=$regopc['id']?>">
                                            Debes proporcionar una respuesta</div>
                                        <?php } ?>
                                    </div>
                                </label>
                            </div>
                            ...
                            <div class="<?=$csscol?> form-check form-check-inline text-start">
                                <?php if ($regpre['tipo']=='CHECK') { ?>
                                <input class="form-check-input" type="<?=$tipo?>" name="pregunta_<?=$regpre['id']?>"
                                    input="<?=$regopc['input']?>" reque="<?=$regopc['requerido']?>"
                                    value="<?=$regopc['id']?>" id="<?=$regopc['id']?>"
                                    onclick="opcOtroCheckClick(<?php echo $regopc['id']; ?>);" autocomplete="off" />
                                <?php } else { ?>
                                ...
                                <label class="form-check-label" for="diariamente"><?=$regopc['opcion']?>
                                    ...
                                    <input type="text"
                                        ...
                                        style="width: auto" placeholder="Coloque su respuesta..."
                                        id="input_<?=$regopc['id']?>" autocomplete="off" />
                                    <div class="invalid-feedback" id="msg_<?=$regpre['id']?>_<?=$regopc['id']?>">Debes
                                        proporcionar una respuesta</div>
                                    <?php } ?>
                                </label>
                            </div>
                            <?php } } ?>
                    ...
                    <div class="invalid-feedback" id="msg_<?=$regpre['id']?>">Debes seleccionar al menos una opción
                    </div>
                    <?php } else { ?>
            ...
            <!-- #pregunta -->
            <span class="position-absolute top-0 start-0 translate-middle badge rounded-circle bg-danger fs-5">
                <?=$regpre['orden'];?> </span>
        </div>
        ...
        <!-- botón de enviar -->
        <div class="mt-4">
            <a class="btn btn-lg ripple center red d-flex align-items-center justify-content-center" href="#"
                id="btnEnviar">
                ENVIAR ENCUESTA
            </a>
            <div class="invalid-feedback" style="color:#14179a;font-size: 1.2em;" id="msg_enviar_error">Favor verificar,
                faltan respuestas requeridas</div>
        </div>
...
$(document).ready(function() {
    ...
    var idenc = "<?=$idenc; ?>";
    var imgfondo =
        "<?php if (isset($encuesta['img_fondo'])) { echo $encuesta['img_fondo']; } else { echo "";} ?>";
    var colortxt =
        "<?php if (isset($encuesta['color_txt'])) { echo $encuesta['color_txt']; } else { echo "";} ?>";
