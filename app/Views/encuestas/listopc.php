    ...
    <div class="text-center">
        ...
        <div class="center mt-3" style="background-color: white;font-family: monospace!important;font-size: small;">
          ...
          <th class="center ">Id Opción</th>-->
                        ...
                        <th class="center ">Clase Imagen</th>
                ...
                <tbody>
                    ...
                    $imgname = '';
                    if($reg['img_opcion'] != ''){
                      $dataImg = explode(";", $reg['img_opcion']);
                      $imgname = $dataImg[0];
                      if(sizeof($dataImg) >= 2){
                        $imgclass = $dataImg[1];
                      }
              ...
              <td class="center"><?=$reg['id']?></td>-->
                        ...
                        <td class="center"><img src="<?=$url_base;?><?=$upload_dir;?>/<?=$imgname;?>"
                                alt="<?=$imgname;?>" style="width:20%;"></td>
                        <td class="center"><?=$imgname?></td>
                        <td class="center"><?=$imgclass?></td>
                        <td class="center"><?=$reg['img_alto']?></td>
                        <td class="center"><?=$reg['img_ancho']?></td>
                        <td class="center"><?=$reg['orden']?></td>
                        <td class="center"><?=$reg['activo']?></td>
                        <td class="center ">
                            <a href="<?=$url_base.'opciones/editopc/'.$reg['id'].'/'.$idp.'/'.$ideencryp;?>"
                                class="btn btn-primary btn-sm" role="button">Editar</a>
                            <a href="<?=$url_base.'opciones/deleteopc/'.$reg['id'].'/'.$idp.'/'.$ideencryp;?>"
            ...
            <div class="text-center">
                ...
                <?php if ($page>0) {?>
                <a href="<?=$url_base.'opciones/'.$page.'/'.$ideencryp.'/'.$idpencryp;?>"
                    class="btn btn-secondary btn-sm" role="button">Ver más</a>
                <?php } else { ?>
...
$(document).ready(function() {
    ...
    $('#list_encuestas').on('change', function(e) {
        ...
        if (ide != "") {
            window.location = url + 'opciones/1/' + ide;
        }
    });
    ...
    $('#list_preguntas').on('change', function(e) {
        ...
        if (idp != "") {
            window.location = url + 'opciones/1/' + ide + '/' + idp;
        }
    });
