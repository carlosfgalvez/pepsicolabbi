...
function validarEncuesta(preg,pregreq,resp,input,inputreq) {
	...
	for (let i = 0; i < pregreq.length; i++) {
		...
		if (req=='S') { // validar que tenga respuesta
			//console.log('pregunta: '+idpreg+' resp: '+resp[i]);
			if (resp[i]==0) {
				$('#msg_'+idpreg).css("display","block");validOk = false;
			}
		}
	}
	...
	for (let i = 0; i < input.length; i++) {
		...
		if (req=='S') { // validar que tenga respuesta
			if (input[i]=='' || input[i]==null) {
				$('#msg_'+idpreg+'_'+idresp).css("display","block");validOk = false;
			}
		}
	}
...
function enviar(idenc,preg,resp,input,url){
			var uid = "";
			if (typeof fp !== "undefined") {uid=fp.get();}  // finger prints
		  var dataString ='acc=1&ide='+idenc+'&pre='+preg+'&res='+resp+'&inp='+input+'&uid='+uid;
		         ...
		         success:function(data) {
		           console.log('enviar resultado: '+data);
                    ...
                    //location.href="gracias?id="+id;
								...
								$("#msg_enviar_error").text('No se pudo enviar la encuesta, vuelva a intentarlo.');
								$('#msg_enviar_error').css("display","block");
							}
            }
		  });
...
function validarEncuestaDatosPersonales(nombre,email,celular,aviso) {
	...
	if (typeof nombre !== 'undefined') {
		if (nombre=="")  { msgError = "Campo obligatorio"; $('#msg_nombre').text(msgError);$('#msg_nombre').css("display","block");validOk = false; }
		if (nombre.length>50)  { msgError = "El máximo de caracteres es de 50"; $('#msg_mombre').text(msgError);$('#msg_mombre').css("display","block");validOk = false; }
		if (email!="" && !emailIsValid (email) ) { msgError = "Debes proporcionar un correo válido"; $('#msg_email').text(msgError);$('#msg_email').css("display","block");validOk = false; }
		if (email.length>50)  { msgError = "El máximo de caracteres permitido es de 50"; $('#msg_email').text(msgError);$('#msg_email').css("display","block");validOk = false; }
		if (email=="")  { msgError = "Campo obligatorio"; $('#msg_email').text(msgError);$('#msg_email').css("display","block");validOk = false; }
		//if (cel=="")  { msgError = "Campo obligatorio"; $('#msgCelular').text(msgError);$('#msgCelular').css("display","block");validOk = false; }
		if (celular.length>45)  { msgError = "El máximo de caracteres es de 45"; $('#msg_celular').text(msgError);$('#msg_celular').css("display","block");validOk = false; }
		if (!aviso)  { msgError = "Debes aceptar el aviso de privacidad"; $('#msg_aviso').text(msgError);$('#msg_aviso').css("display","block");validOk = false; }
	}
...
function validarIngreso(email,pass) {
	...
	if (email=="")  { msgError = "Campo obligatorio"; $('#msgEmail').text(msgError);$('#msgEmail').css("display","block");validOk = false; }
  if (pass=="")  { msgError = "Campo obligatorio"; $('#msgContrasena').text(msgError);$('#msgContrasena').css("display","block");validOk = false; }
	if ((pass.length<8 || pass.length>20))  { msgError = "Debe contener entre 8 y 20 caracteres"; $('#msgContrasena').text(msgError);$('#msgContrasena').css("display","block");validOk = false; }
